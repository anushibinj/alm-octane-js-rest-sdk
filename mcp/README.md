...
- Query helper: `octane_generate_query_string` (infers `fields` + `query` from natural language using ValueEdge query grammar rules, validates syntax locally, then validates via query execution with default `limit=5`, and retries alternate query candidates)
- Query validator: `octane_validate_query_string` (parses `fields=...&query=...` using ValueEdge grammar, validates syntax locally, executes with default `limit=5`, and returns `valid: true|false` with details)
